...
from .aff_fpn import build_aff_fpn
...
def edge_detection(images):
    # image = images_norm.tensor
    # image = image[0].permute(1, 2, 0)
    # import cv2
    # import numpy as np
    # a = np.array(image)
    # cv2.imshow('pic_name', a)
    # cv2.waitKey(0)  # 等待按键
    ...
    for i in range(images.size()[0]):
        ...
        grad = torch.from_numpy(grad).permute(2,0,1)
        images_edge.append(grad)
    images_edge = torch.stack(images_edge, dim=0)
    return images_edge.to(images.device)
...
class CondInst(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.num_grids = cfg.MODEL.FCOS.NUM_GRIDS
        self.use_share_tra = cfg.MODEL.FCOS.USE_SHARE_TRANSFORMERS
        self.num_share_tra = cfg.MODEL.FCOS.NUM_SHARE_TRANSFORMERS

        # build top module
        in_channels = self.proposal_generator.in_channels_to_top_module
        # self.aff_fpn = build_aff_fpn(channels=in_channels, askc_type='AFF')

        # if self.use_share_tra:
        #     self.share_transformer_tower = TwinTransformer(dim=in_channels, depth=self.num_share_tra)
        ...
        # self.pre_seg_convs = nn.ModuleList()
        # for i in range(5):
        #     pre_seg_conv = nn.Sequential(
        #         nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1),
        #         nn.BatchNorm2d(in_channels),
        #         nn.ReLU(),
        #         nn.Conv2d(in_channels, 1, kernel_size=1, stride=1, padding=0),
        #     )
        #     self.pre_seg_convs.append(pre_seg_conv)
    ...
    def forward(self, batched_inputs):
        ...
        images_edge = edge_detection(original_images)
        ...
        # features = self.aff_fpn(features)
        # if self.use_share_tra:
        #     features = self.share_transformers(features)
        ...
        mask_feats, sem_losses = self.mask_branch(features, gt_instances=gt_instances, residual_features=None)
        # loss_pre = self.pre_segmentation(features, gt_instances=gt_instances)
        ...
        if self.training:
            mask_losses = self._forward_mask_heads_train(proposals, mask_feats, gt_instances, images_edge=images_edge)
            ...
            pred_instances_w_masks = self._forward_mask_heads_test(proposals, mask_feats, images_edge=images_edge)
    ...
    def _forward_mask_heads_train(self, proposals, mask_feats, gt_instances, images_edge=None):
        ...
        loss_mask = self.mask_head(
            ...
            pred_instances, gt_instances, images_edge=images_edge
        )

        return loss_mask

    def _forward_mask_heads_test(self, proposals, mask_feats, images_edge=None):
        # prepare the inputs for mask heads
        ...
        pred_instances_w_masks = self.mask_head(
            mask_feats, self.mask_branch.out_stride, pred_instances, images_edge=images_edge
        )

        return pred_instances_w_masks

    def pre_segmentation(self, features, gt_instances=None):
        gt_masks = []
        if gt_instances:
            for per_im in gt_instances:
                gt_mask = per_im.gt_bitmasks_full
                gt_mask = torch.sum(gt_mask, dim=0)
                a = torch.ones_like(gt_mask)
                b = torch.zeros_like(gt_mask)
                gt_mask = torch.where(gt_mask>0., a, b)
                ...
                cv2.imshow('origsa', m)
                cv2.waitKey(0)  # 等待按键
            gt_masks = torch.stack(gt_masks, dim=0).unsqueeze(dim=1)
        ...
        for i in range(len(layers)):
            gt_m = gt_masks.clone()
            x = features[layers[i]]
            pre_mask = self.pre_seg_convs[i](x)
            b, c, h, w = pre_mask.size()
            gt_m = F.interpolate(gt_m, size=(h, w), mode='bilinear', align_corners=True)
            pro_loss = F.binary_cross_entropy_with_logits(pre_mask, gt_m)
            pre_losses.append(pro_loss)

        return pre_losses

    def share_transformers(self, features):
        out_features = []
        out_layers = []
        for l, layer in enumerate(features):
            ...
            seg_num_grid = self.num_grids[l]
            feature = F.interpolate(feature, size=seg_num_grid, mode='bilinear')
            feature = self.share_transformer_tower(feature)
            feature = F.interpolate(feature, size=(h, w), mode='bilinear')
            out_features.append(feature)
        return dict(zip(out_layers, out_features))
