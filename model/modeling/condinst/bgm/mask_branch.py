...
class MaskBranch(nn.Module):
    ...
    def forward(self, features, gt_instances=None, residual_features=None):
        #self.in_features: ["p3", "p4", "p5"]
        for i, f in enumerate(self.in_features):
            if residual_features==None:
                feature = features[f]
            else:
                feature = torch.cat([residual_features[f], features[f]], dim=1)
            if i == 0:
                x = self.refine[i](feature)
            else:
                x_p = self.refine[i](feature)
