...
class DynamicMaskHead(nn.Module):
    ...
    def mask_heads_forward(self, features, weights, biases, num_insts):
        ...
        for i, (w, b) in enumerate(zip(weights, biases)):
            ...
            # if i < n_layers - 1:
                # x = F.relu(x)
                # x, score_map = self.spatialattentions[i](spatial_features, x)
    ...
    def mask_heads_forward_with_coords(
        ...
        mask_pre = self.prediction(mask_logits)
    ...
    def __call__(self, mask_feats, mask_feat_stride, pred_instances, gt_instances=None, images_edge=None):
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                mask_pre, mask_logits = self.mask_heads_forward_with_coords(
                ...
                if self.boxinst_enabled:
                    ...
                    mask_losses = dice_coefficient(pre_mask_scores, gt_bitmasks)
...
def ECE_loss(mask_scores, gt_bitmasks):
    ...
    entirety_loss = F.binary_cross_entropy_with_logits(mask_scores, gt_bitmasks, weight=gt_edges, reduction="mean")


    return entirety_loss
...
class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=3):
        ...
        self.feats_fusion = nn.Sequential(
    ...
    def forward(self, features, x):
        ...
        map = self.conv(x)
        map = F.sigmoid(map)
        x = features * map.expand_as(features)
        x = residual + x
        x = x.reshape(1, -1, H, W)
        return x, map
