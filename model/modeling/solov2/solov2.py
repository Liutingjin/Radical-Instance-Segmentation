...
class SOLOv2(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        """
        images, images_edge = self.preprocess_image(batched_inputs)
        ...
        if self.training:
            ...
            losses = self.loss(cate_pred, kernel_pred, mask_pred, targets, images_edge=images_edge)
            return losses
        else:
            # point nms.
            ...
            results = self.inference(cate_pred, kernel_pred, mask_pred, images.image_sizes, batched_inputs, images_edge=images_edge)
            return results
    ...
    def preprocess_image(self, batched_inputs):
        ...
        images_edge = edge_detection(images)

        images = [self.normalizer(x) for x in images]
        images = ImageList.from_tensors(images, self.backbone.size_divisibility)
        return images,images_edge
        # return images
    ...
    def loss(self, cate_preds, kernel_preds, ins_pred, targets, images_edge=None):
        pass
        ...
        b = len(ins_label_list)
        b_kernel_preds = [[] for _ in range(b)]
        for b_kernel_pred in kernel_preds:
            for idx, bs_kernel_pred in enumerate(b_kernel_pred):
                if bs_kernel_pred.size()[-1] == 0:
                    continue
                b_kernel_preds[idx].append(bs_kernel_pred)
        ins_pred_list = []
        for idx in range(b):
            if len(b_kernel_preds[idx]) == 0:
                continue
            cur_ins_pred = ins_pred[idx, ...]
            H, W = cur_ins_pred.shape[-2:]
            cur_ins_pred = cur_ins_pred.unsqueeze(0)

            kernel_pred = torch.cat(b_kernel_preds[idx], 1)
        ...
        targets = []
        input_egm = []
        for input, input1, target in zip(ins_pred_list, mask_logits, ins_label_list):
            if input is None:
                continue
            ...
            loss_ins_egm.append(dice_loss(input1, target))
            input_egm.append(input1)
            targets.append(target)
        loss_ins_mean = torch.cat(loss_ins).mean()
        loss_ins = loss_ins_mean * self.ins_loss_weight
        loss_ins_egm_mean = torch.cat(loss_ins_egm).mean()
        #BCE
        targets = torch.cat(targets, 0)
        input_egm = torch.cat(input_egm, 0)
        loss_ins_egm_bce = F.binary_cross_entropy(input_egm, targets.to(dtype=torch.float32), reduction="mean")
        loss_ins_fine = (loss_ins_egm_mean + loss_ins_egm_bce) * self.ins_loss_weight
    ...
    def inference(self, pred_cates, pred_kernels, pred_masks, cur_sizes, images, images_edge=None):
        ...
        for img_idx in range(len(images)):
            ...
            result = self.inference_single_image(pred_cate, pred_kernel, pred_mask,
                                                 cur_sizes[img_idx], ori_size, images_edge=images_edge)
            results.append({"instances": result})
        return results

    def inference_single_image(
            self, cate_preds, kernel_preds, seg_preds, cur_size, ori_size, images_edge=None
    ):
        # overall info.
        h, w = cur_size
...
class SOLOv2InsHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        assert in_channels == cfg.MODEL.SOLOV2.INSTANCE_IN_CHANNELS, \
            print("In channels should equal to tower in channels!", in_channels, cfg.MODEL.SOLOV2.INSTANCE_IN_CHANNELS)
