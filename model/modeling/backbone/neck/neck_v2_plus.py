...
##保留FPN结构，再进行HIM, FPN之后的特征是P2,P3,P4,P5，对4层特征进行融合。
class MyFPN(Backbone):
    ...
    def cfg(self, x, group_convs, cascade_convs):
        ...
        for i in range(len(x)):
            ...
            y = y.split((64, 64, 64, 64), 1)
            results_d.append(y)

        for idx, (x_c2, x_c3, x_c4, x_c5) in enumerate(zip(results_d[0], results_d[1], results_d[2], results_d[3])):
            if idx == 0:
                x_c2 = F.interpolate(x_c2, scale_factor=1/8.0, mode="nearest")
                x_c3 = F.interpolate(x_c3, scale_factor=1/4.0, mode="nearest")
                x_c4 = F.interpolate(x_c4, scale_factor=1/2.0, mode="nearest")
            elif idx == 1:
                x_c2 = F.interpolate(x_c2, scale_factor=1 / 4.0, mode="nearest")
                x_c3 = F.interpolate(x_c3, scale_factor=1 / 2.0, mode="nearest")
                x_c5 = F.interpolate(x_c5, scale_factor=2.0, mode="nearest")
            elif idx == 2:
                x_c2 = F.interpolate(x_c2, scale_factor=1 / 2.0, mode="nearest")
                x_c4 = F.interpolate(x_c4, scale_factor=2.0, mode="nearest")
                x_c5 = F.interpolate(x_c5, scale_factor=4.0, mode="nearest")
            else:
                x_c3 = F.interpolate(x_c3, scale_factor=2.0, mode="nearest")
                x_c4 = F.interpolate(x_c4, scale_factor=4.0, mode="nearest")
                x_c5 = F.interpolate(x_c5, scale_factor=8.0, mode="nearest")

            p = torch.cat((x_c2, x_c3, x_c4, x_c5), dim=1)
